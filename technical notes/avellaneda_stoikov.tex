...
\end{equation*}
The $-\gamma x$ terms cancel. Rearranging,
\begin{equation*}
\gamma r^b = \gamma s - \tfrac12\gamma^2\sigma^2(T-t)\big[(q+1)^2-q^2\big]
 = \gamma s - \tfrac12\gamma^2\sigma^2(T-t)(2q+1),
\end{equation*}
and dividing by $\gamma$ gives Equation~\eqref{eq:rb}. The identical argument applied to $V(x+r^a,s,q-1,t)=V(x,s,q,t)$ gives
\begin{equation*}
-\gamma r^a = -\gamma s + \tfrac12\gamma^2\sigma^2(T-t)\big[q^2-(q-1)^2\big]
 = -\gamma s+\tfrac12\gamma^2\sigma^2(T-t)(2q-1),
\end{equation*}
which rearranges to Equation~\eqref{eq:ra}.
\end{proof}
...
\end{theorem}
\begin{proof}
Using Assumption~\ref{ass:arrivals}, write
\begin{equation*}
f(\delta) = \Lambda(\delta)(1-e^{-\gamma\delta})
 = A\big(e^{-\kappa\delta}-e^{-(\kappa+\gamma)\delta}\big).
\end{equation*}
Then
\begin{equation*}
