...
\end{theorem}
\begin{proof}
Unrolling Equation~\eqref{eq:price}, and using $\tau g(n_i/\tau)=\gamma n_i$ under Assumption~\ref{ass:linear},
\begin{equation*}
S_{k-1} = S_0 - \gamma\sum_{i=1}^{k-1} n_i + \sigma\sqrt\tau\sum_{i=1}^{k-1}\xi_i,
\end{equation*}
so $E[S_{k-1}] = S_0-\gamma\sum_{i=1}^{k-1}n_i$ since $E[\xi_i]=0$, and Equation~\eqref{eq:execprice} in turn gives
\begin{equation*}
E[\tilde S_k] = S_0 - \gamma\sum_{i=1}^{k-1}n_i - \frac{\eta n_k}{\tau}.
\end{equation*}
Substituting into Equation~\eqref{eq:ISdef} and using $\sum_k n_k = X$,
\begin{equation*}
...
Since
\begin{equation*}
\left(\sum_k n_k\right)^{\!2} = \sum_k n_k^2 + 2\sum_{i<k}n_in_k = X^2,
\end{equation*}
the cross-sum equals $\tfrac12\left(X^2-\sum_k n_k^2\right)$, giving Equation~\eqref{eq:meanIS} directly.
\end{proof}
...
\label{eq:trajectory}
\end{equation}
where $\tilde\kappa>0$ solves
\begin{equation}
\cosh(\tilde\kappa\tau) = 1+\frac{\lambda\sigma^2\tau^2}{2\tilde\eta}.
\label{eq:kappadef}
\end{equation}
\end{theorem}
\begin{proof}
...
\end{equation*}
Writing $n_j=x_{j-1}-x_j$ and $n_{j+1}=x_j-x_{j+1}$, this is
\begin{equation*}
\tilde\eta\,(n_{j+1}-n_j) = -\lambda\sigma^2\tau\,x_j.
\end{equation*}
Substituting
\begin{equation*}
n_{j+1}-n_j = (x_j-x_{j+1})-(x_{j-1}-x_j) = 2x_j-x_{j+1}-x_{j-1}
\end{equation*}
and rearranging,
\begin{equation}
...
\label{eq:diffeq}
\end{equation}
a linear, constant-coefficient second-order difference equation with characteristic roots $e^{\pm\tilde\kappa\tau}$ (the definition of $\cosh(\tilde\kappa\tau)$ guarantees $\tilde\kappa$ is a well-defined positive real number, since the right-hand side exceeds $1$ whenever $\lambda,\sigma,\tilde\eta>0$). The general solution is
\begin{equation*}
x_j = \alpha\, e^{\tilde\kappa t_j}+\beta\, e^{-\tilde\kappa t_j}
\end{equation*}
for constants $\alpha,\beta$ fixed by the boundary conditions $x_0=X$, $x_N=0$. It is more convenient to write that same general solution as $x_j=A\sinh(\tilde\kappa(T-t_j))$ for a single constant $A$, which is a valid re-parametrization: $\sinh(\tilde\kappa(T-t_j))$ is itself a linear combination of $e^{\tilde\kappa t_j}$ and $e^{-\tilde\kappa t_j}$, and it already satisfies Equation~\eqref{eq:diffeq} by the hyperbolic identity
\begin{equation*}
\sinh(a-\tilde\kappa\tau)+\sinh(a+\tilde\kappa\tau) = 2\sinh(a)\cosh(\tilde\kappa\tau),
\end{equation*}
applied with $a=\tilde\kappa(T-t_j)$. In this form the condition $x_N=0$ is automatic, since $\sinh(0)=0$, and $x_0=X$ fixes $A=X/\sinh(\tilde\kappa T)$, giving Equation~\eqref{eq:trajectory}.
\end{proof}
...
\end{corollary}
\begin{proof}
As $\tilde\kappa\to0$, $\sinh(\tilde\kappa u)/u \to \tilde\kappa$ for any fixed $u$, this being the derivative of $\sinh$ at $0$, so
\begin{equation*}
\frac{\sinh(\tilde\kappa(T-t_j))}{\sinh(\tilde\kappa T)}
= \frac{\sinh(\tilde\kappa(T-t_j))/(T-t_j)}{\sinh(\tilde\kappa T)/T}\cdot\frac{T-t_j}{T}
\;\longrightarrow\; \frac{T-t_j}{T} = 1-\frac{j}{N}. \qedhere
\end{equation*}
\end{proof}
...
\end{corollary}
\begin{proof}
Fix $j\ge1$. As $\tilde\kappa\to\infty$,
\begin{equation*}
\frac{\sinh(\tilde\kappa(T-t_j))}{\sinh(\tilde\kappa T)} \sim e^{-\tilde\kappa t_j} \;\longrightarrow\; 0,
\end{equation*}
since both numerator and denominator are asymptotically dominated by their positive exponential term, and $T-t_j<T$ strictly for $j\ge1$, so the ratio of exponentials vanishes.
\end{proof}
...
\end{corollary}
\begin{proof}
For small $\tilde\kappa\tau$, $\cosh(\tilde\kappa\tau) - 1 \approx \tfrac12(\tilde\kappa\tau)^2$. Equating this to the defining relation~\eqref{eq:kappadef} of Theorem~\ref{thm:trajectory},
\begin{equation*}
\tfrac12\,\tilde\kappa^2\tau^2 \approx \frac{\lambda\sigma^2\tau^2}{2\tilde\eta},
\qquad\text{so}\qquad
\tilde\kappa^2\to\frac{\lambda\sigma^2}{\tilde\eta}.
\end{equation*}
Rescaling so that $\eta$ itself (rather than $\eta/\tau$) is held fixed as $\tau\to0$, $\tilde\eta=\eta/\tau-\gamma/2\to\eta/\tau$ diverges at the same rate needed to keep $\tilde\eta\tau\to\eta$ finite, and $\gamma\tau/2\to0$ is negligible by comparison. Substituting $\tilde\eta\to\eta/\tau$ into $\tilde\kappa^2\tau^2\to\lambda\sigma^2\tau^2/\tilde\eta$ then gives
\begin{equation*}
\tilde\kappa^2 \;\longrightarrow\; \frac{\lambda\sigma^2}{\eta}. \qedhere
\end{equation*}
\end{proof}
