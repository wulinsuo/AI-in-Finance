...
\section{The Problem: Where Do Population Shares Come From?}
\label{sec:problem}
...
Under (A5),
\begin{equation*}
p_t^* = \sum_{k=1}^{\infty} R^{-k}E_t[y_{t+k}].
\end{equation*}
\end{lemma}
\begin{proof}
By induction on $K$, $p_t^*=\sum_{k=1}^{K}R^{-k}E_t[y_{t+k}]+R^{-K}E_t[p_{t+K}^*]$. The base case $K=1$ is (A5)'s recursion divided by $R$. For the inductive step, apply (A5) at date $t+K$ and the law of iterated expectations,
\begin{equation*}
E_t[p_{t+K}^*]=E_t\big[R^{-1}E_{t+K}[p_{t+K+1}^*]+R^{-1}E_{t+K}[y_{t+K+1}]\big]
=R^{-1}E_t[p^*_{t+K+1}]+R^{-1}E_t[y_{t+K+1}],
\end{equation*}
which extends the sum to $K+1$ terms. Letting $K\to\infty$ and invoking the transversality condition of (A5) gives the stated result.
\end{proof}
...
\end{equation}
Equations~\eqref{eq:twotypeprice}--\eqref{eq:twotypedelta} together with Lemma~\ref{lem:logit} define a second-order nonlinear recursion in the single observable $x_t$,
\begin{equation*}
x_t=\Phi(x_{t-1},x_{t-2}).
\end{equation*}
