...
Two features of Table~\ref{tab:km_table} are worth pausing on. First, the number at risk, $n_i$, drops not only when a fraud closure occurs but also whenever a censored observation passes that day, which is why $n_i$ falls from 12 to 9 between day 5 and day 12 even though only two confirmed closures occurred in between (day 8's censored account and no additional confirmed closures reduce the count without contributing a $d_i$ term).
...
\subsubsection*{Extending Layering Detection to the Blockchain: Clustering and Mixing Services}\index{Blockchain Forensics}
...
The layering technique the chain illustrates, however, transfers directly onto this pseudonymous ledger, and the first tool blockchain forensics analysts use to unwind it is the network-clustering logic Figure~\ref{fig:layering_network} already applies to named entities, adapted to work from raw addresses alone.
...
Applying this rule repeatedly across many transactions merges addresses into clusters, the way Table~\ref{tab:layering_network}'s four names group what could otherwise have been dozens of legally separate shell accounts. A small worked example makes this concrete. Suppose an investigator observes eight raw addresses, A1 through A8, and five transactions that each jointly spend two of them as inputs,
\begin{equation*}
(A1, A2), \quad (A2, A3), \quad (A4, A5), \quad (A6, A7), \quad (A7, A8).
\end{equation*}
Treating each pair as an edge and taking connected components, the union-find structure standard in blockchain-clustering software, collapses these eight addresses into three inferred real-world entities,
\begin{equation*}
...
A detection model's value is decided by the cost of its false positives and by who is accountable for it.
