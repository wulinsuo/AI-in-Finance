...
\subsection{Stationarity, White Noise, and Random Walks}
...
This section and the two that follow all draw on a single running example, given in Table~\ref{tab:ar1_data}: eight monthly observations of a short-term interest-rate deviation from its policy target, measured in basis points. The series is deliberately tiny, short enough that every calculation in it can be checked by hand; Section~\ref{sec:acf-section} computes its correlogram and Section~\ref{sec:ar1-example} fits an AR(1) model to it.
...
Applying the (non-augmented) test to this series means regressing $\Delta X_t$ on $X_{t-1}$:
\begin{equation*}
\hat\gamma = -0.290, \qquad SE(\hat\gamma) = 0.259, \qquad t\text{-statistic} = \frac{\hat\gamma}{SE(\hat\gamma)} \approx -1.12.
\end{equation*}
Notice that $\hat\gamma = -0.290$ is consistent with the AR(1) coefficient fitted in Section~\ref{sec:ar1-example}, since
...
Since $|-1.12| < 2.89$, this test fails to reject the null hypothesis of a unit root, even though the AR(1) coefficient of 0.71 and the visibly decaying correlogram in Figure~\ref{fig:acf} both suggest genuine mean reversion. This apparent contradiction is not a flaw in the test; it is a direct consequence of the sample size: with only 8 observations, the Dickey-Fuller test has very little statistical power to distinguish a stationary process from a unit-root process, the ``limited effective sample size'' challenge flagged later in this chapter. In practice, the ADF test is typically applied to series with many more observations than this illustrative example, where its power to detect stationarity is much greater.
...
\subsection{Autocorrelation and the Correlogram}\index{Autocorrelation and the Correlogram}
...
This part builds the classical forecasting families in order, from a single autoregressive term to the full ARIMA specification.

\subsection{A Worked Example: Fitting and Forecasting an AR(1) Model}
...
The data are the running series introduced in Section~\ref{sec:adf-test}, the eight monthly interest-rate deviations of Table~\ref{tab:ar1_data}.

The sample autocorrelations, computed directly from the formula above using the full-sample mean $\bar X = 6.25$, are $\hat\rho_1 = 0.50$, $\hat\rho_2 = 0.24$, and $\hat\rho_3 = 0.03$, plotted in Figure~\ref{fig:acf}. The roughly geometric decay, each lag's autocorrelation is a bit less than half the previous one, is the signature an AR(1) process should produce.
...
\subsubsection*{Confidence Intervals and $p$-Values for Model Parameters}
\label{sec:ci-pvalue}
...
Chapter 6's Section 6.3.1 already flagged the two practical warnings that apply here without modification: statistical significance is not the same as economic or financial significance, and testing many coefficients or candidate models against the same data inflates the chance that at least one spuriously crosses the $5\%$ threshold by pure luck.
That second warning is the multiple-testing problem Section~\ref{sec:cointegration}'s discussion of pairs selection revisits concretely, fulfilling the forward pointer Chapter 6 itself made to this section: a single $p$-value below $0.05$ is far less persuasive when it is the best of twenty candidate pairs tried rather than the only pair considered.

\subsection{Multi-Step Forecasting and the Growth of Uncertainty}
...
\subsection{Moving Averages and Exponential Smoothing}
...
One series is rarely the whole story; this part models several jointly, and extracts the few factors that drive them.

\subsection{Cointegration and Pairs Trading}\index{Cointegration and Pairs Trading}
...
Because the strategy trades the spread rather than the direction of either stock, it is designed to profit regardless of whether the overall market rises or falls, a property called market neutrality, provided the cointegrating relationship itself remains stable, which is the kind of assumption that can silently break down, a risk explored further when Chapter 13 discusses the practical risks of statistical arbitrage strategies.

\subsection{Vector Autoregression: Modeling Several Series Jointly}
...
\section{Volatility and State-Space Models}

Volatility is itself predictable in a way the level is not, and some of what matters is never observed directly.

\subsection{Volatility Clustering and the GARCH Family}
\label{sec:garch}
...
whose sample variance was $0.000414$. Take this as the seed $\sigma_1^2$, and suppose (for illustration, not estimated from only four data points) that $\omega = 0.00002$, $\alpha = 0.10$, and $\beta = 0.85$, so that $\alpha+\beta = 0.95$ and the long-run variance is $\omega/(1-0.95) = 0.0004$, a long-run volatility of $2.00\%$. The recursion needs shocks rather than returns, and the two are not quite the same thing: a GARCH model sits on top of a mean equation $r_t = \mu + \varepsilon_t$, so the shock is properly the deviation of the return from its expected value, $\varepsilon_t = r_t - \mu$. Setting $\mu = 0$ lets the returns themselves stand in for the shocks, which is the conventional treatment for daily data, where the average drift is smaller than a typical daily move by two orders of magnitude and makes almost no difference to the fitted variance path. Applying the recursion with $\varepsilon_{t-1} = r_{t-1}$ on that basis gives Table~\ref{tab:garch_recursion}.
...
\subsection{State-Space Models and the Kalman Filter}
...
Time order makes the standard validation split invalid, so this part replaces it and asks what a forecast should be scored against.

\subsection{Walk-Forward Validation for Time Series}
...
The newest entrant in this debate sidesteps the limited-sample objection in an interesting way. \textbf{Time-series foundation models}\index{time-series foundation models}, large neural sequence models pretrained once on enormous corpora of diverse time series (retail demand, energy load, traffic, weather, and much else) and then applied to a new series with little or no additional fitting, arrived in the mid-2020s under names like Chronos and TimesFM, borrowing directly from the pretrain-then-adapt recipe Chapter 16 describes for language models. Because the heavy parameter fitting happened on the pretraining corpus rather than on the forecaster's own short series, the overfitting arithmetic above changes: the model brings pattern knowledge in rather than extracting it all from the sample at hand, and on generic forecasting benchmarks these models produce respectable zero-shot forecasts of series they have never seen. What has not changed is anything specific to finance: published evaluations to date show them competitive with, not dominant over, well-tuned classical baselines on financial series, whose low signal-to-noise ratio and regime shifts are the features a pretraining corpus of mostly non-financial series does little to prepare a model for. A practitioner should treat such a model as one more candidate forecaster to be judged inside the same walk-forward, benchmark-against-naive discipline this chapter built, not as an exemption from it.

\section{A Practical Workflow for Building a Time Series Model}
...
\centering
\begin{lrbox}{\bookmeasuredbox}
    % Reduced from scale=0.9 / node distance=1.3cm / minimum width=2.6cm. The gaps
    % were doing most of the damage: at 1.3cm the seven boxes spanned ~85% of the
    % text width. Shrinking the gaps costs nothing legible; the scale step is kept
    % small so the two-line labels stay readable.
    \begin{tikzpicture}[>=latex,thick,node distance=0.8cm,scale=0.85,transform shape]
        \node[draw,rounded corners,minimum width=2.4cm,minimum height=0.8cm,align=center] (raw) {Raw series};
        \node[draw,rounded corners,minimum width=2.4cm,minimum height=0.8cm,right=of raw,align=center] (stationarity) {Test\\stationarity};
        \node[draw,rounded corners,minimum width=2.4cm,minimum height=0.8cm,right=of stationarity,align=center] (identify) {Identify\\order (ACF/PACF)};
        \node[draw,rounded corners,minimum width=2.4cm,minimum height=0.8cm,right=of identify,align=center] (fit) {Fit\\candidate models};
        \node[draw,rounded corners,minimum width=2.4cm,minimum height=0.8cm,below=0.8cm of fit,align=center] (validate) {Walk-forward\\validate};
        \node[draw,rounded corners,minimum width=2.4cm,minimum height=0.8cm,left=of validate,align=center] (select) {Select model\\(AIC/BIC, RMSE)};
        \node[draw,rounded corners,minimum width=2.4cm,minimum height=0.8cm,left=of select,align=center] (monitor) {Deploy and\\monitor};
...
The cointegration and VAR material extended these ideas from a single series to relationships between series, previewing the pairs-trading strategies of Chapter 13 and the multivariate risk factor models of Chapter 8.
...
\begin{enumerate}
    ...
    \item Using the multi-step forecasting formulas in Section 7.3.2, compute the 4-step-ahead AR(1) forecast $\hat X_{12}$ from $X_8=4.0$, and verify it lies between the 3-step and 5-step forecasts already reported in Table~\ref{tab:multistep}.
    \item Using Holt's linear trend method from Section 7.3.3 with $\alpha=0.5$ and $\beta=0.2$ (instead of $\alpha=0.4$, $\beta=0.3$), recompute the level and trend after processing $X_2=8.0$ through $X_4=7.0$ (three updates starting from $L_1=9.0$, $T_1=-1.0$), and compare your intermediate values to those reported in the chapter.
...
\begin{itemize}
    ...
    \item Ansari et al., ``Chronos: Learning the Language of Time Series,'' \emph{Transactions on Machine Learning Research}. The paper introducing one of the pretrained time-series foundation models discussed in this chapter's closing challenges, applying the language-model pretraining recipe to zero-shot forecasting.
