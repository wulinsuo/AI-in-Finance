...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[>=latex,thick,node distance=0.7cm]
        ...
        \draw[->] (model) -- (app);
    \end{tikzpicture}
    \caption{The text-to-application pipeline this chapter develops. Vectorization (Section~\ref{sec:bow-tfidf} and Section~\ref{sec:embeddings}) and modeling (Section~\ref{sec:naive-bayes-text} and Section~\ref{sec:attention-transformers}) are where most of the chapter's technical content sits.}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[>=latex,thick,node distance=1.1cm and 1.6cm]
        ...
        \draw[->] (attn) -- (out);
    \end{tikzpicture}
    \caption{The self-attention computation as a diagram. The same token embedding $x$ is projected three ways through independently learned weight matrices $W^Q$, $W^K$, $W^V$; the resulting query, key, and value vectors are then combined by the attention formula into a single contextual representation.}
...
Two refinements turn this single calculation into the mechanism actually used in practice. First, a real transformer layer computes several 
...
After next-token pretraining, a model is typically further trained in two stages: supervised fine-tuning on curated (prompt, ideal response) pairs, 
