...
\subsection{Supervised and Unsupervised Learning}\index{Supervised and Unsupervised Learning}
...
This part builds the standard supervised toolkit from the ground up, each model worked numerically before the next is introduced.

\subsection{Regression: A Worked Example}
...
\subsubsection*{Standard Errors, Confidence Intervals, and $p$-Values for Coefficients}
...
\usebox{\bookmeasuredbox}
    ...
    \caption{The idea of gradient descent. Starting from an initial guess $\theta_0$, each step moves against the local slope of the loss curve, the negative gradient, illustrated here by the tangent line at $\theta_0$, producing a sequence $\theta_0,\theta_1,\theta_2,\ldots$ that descends toward the minimizer $\theta^\star$. Steps are large where the curve is steep and shrink automatically as the curve flattens near the minimum, the pattern seen numerically in Table~\ref{tab:gradient_descent} below.}
...
\subsection{Classification: A Worked Example}
...
\end{equation*}
A remark on terminology worth one sentence, because the reader will meet it in industry: linear and logistic regression are both members of the generalized linear model\index{generalized linear model (GLM)} (GLM) family, which pairs a linear combination of features with a link function and an outcome distribution suited to the target, identity link with a normal outcome for ordinary regression, log-odds link with a binary outcome here, and, in the family's other widely-used members, a log link with Poisson-distributed counts or Gamma-distributed positive amounts; GLMs remain the regulatory workhorse of insurance pricing precisely because their fitted coefficients read as multiplicative rating factors.
...
\subsection{$K$-Nearest Neighbors: A Non-Parametric Classifier}
...
\subsection{Support Vector Machines}\index{support vector machine (SVM)}
...
\section{Generalization and Model Selection}

A model that fits its training data perfectly has usually learned the noise; this part covers how that failure is detected, measured, and controlled.

\subsection{Overfitting and Generalization}\index{Overfitting and Generalization}
...
\subsubsection*{A Worked Example: Ridge Versus Lasso on a Genuine and a Spurious Feature}
...
\subsection{Bias, Variance, and the Tradeoff}
...
\subsection{Model Evaluation}\index{Model Evaluation}
...
\centering
\begin{lrbox}{\bookmeasuredbox}
    % scale + transform shape, as in Figure~\ref{fig:toy_feedforward_network}:
    % box sizes, gaps, and the feedback arc all shrink together.
    \begin{tikzpicture}[>=latex,thick,node distance=0.9cm,scale=0.82,
        ...
        % 0.5cm whatever the chain's width.
        \draw[->,rounded corners=3pt] (monitor.north) -- ++(0,0.5) -| (train.north);
    \end{tikzpicture}
\end{lrbox}
\ifdim\wd\bookmeasuredbox<0.65\linewidth
\setlength{\bookcapwidth}{0.65\linewidth}
...
\subsection{Feature Engineering for Financial Data}
...
\setlength{\bookcapwidth}{\wd\bookmeasuredbox}
\fi
% Tightens the object-to-caption gap for this figure only (floatrow's default is
% 10pt). It MUST be floatrow's captionskip: floatrow zeroes \abovecaptionskip
% inside its own box, so \setlength{\abovecaptionskip} here does nothing at all.
...
\subsection{Naive Bayes: A Probabilistic Classifier}
...
\subsection{Neural Networks and Deep Learning in Finance}

Suppose an analyst suspects that default risk depends on leverage and profitability not through any simple additive combination, but through some more intricate interaction between the two that is too complex to write down as a specific nonlinear formula in advance, and wants a model flexible enough to learn whatever that relationship turns out to be directly from data, rather than committing in advance to a particular functional form the way linear or logistic regression do. Neural networks represent another major class of machine learning methods, built for this kind of unspecified nonlinearity. A simple feedforward network with one hidden layer computes
\begin{equation*}
\hat y = \sigma_2\big(W_2\, \sigma_1(W_1 x + b_1) + b_2\big),
\end{equation*}
where $W_1, b_1, W_2, b_2$ are learned weight matrices and bias vectors, and $\sigma_1, \sigma_2$ are nonlinear activation functions, such as the rectified linear unit $\text{ReLU}(z) = \max(0,z)$ for hidden layers or, for a binary output layer, the sigmoid function\index{Sigmoid Function}
\begin{equation*}
\sigma(z) = \frac{1}{1+e^{-z}},
\end{equation*}
which maps any real number into the interval $(0,1)$ so that the output can be read as a probability, and which is the same function logistic regression applied in Section~\ref{sec:classification-example} above. The subscripted $\sigma_1$ and $\sigma_2$ stand for whichever activation a given layer happens to use; a bare $\sigma(\cdot)$, here and in every later chapter, means this sigmoid specifically. Stacking more such layers produces a deep network. The weights are learned by minimizing a loss function (mean squared error for regression, log-loss for classification) using gradient descent, with gradients computed efficiently via backpropagation\index{backpropagation}, an efficient algorithm for computing the gradient of the loss with respect to every weight in the network by applying the chain rule layer by layer from the output back to the input.
...
A single gradient descent step with learning rate $\eta=0.1$, the update rule Section 6.3.2 introduced, moves every parameter a small step opposite its gradient, $W_2 \leftarrow W_2 - \eta\,\partial L/\partial W_2$ and likewise for $b_2$, $W_1$, and $b_1$, nudging the network's prediction for this particular example fractionally closer to the target $y=1$, and repeating this update across many training examples and many iterations is, in its entirety, how a deep network's weights are actually learned.

\begin{figure}[htbp]
\centering
\begin{lrbox}{\bookmeasuredbox}
    % scale + transform shape keeps every gap proportional, so shrinking the
    % diagram cannot make the edge labels collide with the neurons.
    \begin{tikzpicture}[>=latex,thick,scale=0.82,
        every node/.append style={transform shape},
...
\usebox{\bookmeasuredbox}
    ...
    \caption{The toy feedforward network worked through above. Chapter 18 revisits it to verify LIME and integrated-gradients explanations against its exact analytical gradient.}
...
\textbf{Hyperparameter and model-selection overfitting.} Section 6.4.3's grid-search workflow, if applied carelessly, is itself a source of overfitting: searching over many hyperparameter combinations or model families and reporting only the best cross-validated result inflates the reported performance in the same way as the multiple-testing problem above, just one level removed from the original features and trading rules.
...
\begin{enumerate}
    ...
    \item Discuss one reason a model that performs well in-sample or in a backtest may fail out of sample or in live trading, drawing on at least two of the challenges described in Section 6.7 (``Challenges of Machine Learning in Finance'').
    ...
    \item Choose one hyperparameter from any model in this chapter (for example, $\lambda$, $k$, or the number of boosting trees) and describe a grid of candidate values you would try, along with why searching too wide a grid could itself introduce overfitting risk, using Section 6.4.3 and Section 6.7's discussion of model-selection overfitting.
