...
\index{IEEE-CIS Fraud Detection dataset}Genuinely labeled fraud data is scarce, since financial institutions rarely release real transaction-level fraud labels, which is why the two datasets in Table~\ref{tab:appendix-fraud} recur so often in both industry benchmarks and this book's own Chapter 15 and Chapter 21 capstone list. Operational-risk loss data proper is even scarcer: the industry's event-level loss databases (the ORX consortium foremost among them) are member-only, which is why Chapter 10's real-data challenge falls back on the public enforcement and complaint text sources in the same table, regulatory documents rich in the operational-loss and misconduct narratives that a bank's internal incident data contains.
...
\textbf{Point-in-time and look-ahead bias.} Most free APIs return a security's or company's data as it is known \emph{today}, not as it was actually known on the date being studied. Fundamentals get restated after the fact, index membership changes, and a company's own name or ticker can change; naively joining ``current'' fundamentals to historical prices silently leaks future information into a backtest, the look-ahead-bias problem Chapter 17 raises directly.
