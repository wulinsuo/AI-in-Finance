...
WRDS: CRSP, TAQ & Institutional subscription (typically via a university) & Web query tool, plus SAS/Python/R client & CRSP is the academic gold standard for US equity returns; TAQ provides tick-by-tick trade and quote data used in Chapter 14's microstructure examples \\
...
Source & Access & API / library & Notes \\
\midrule
SEC EDGAR & Free & REST API (\url{data.sec.gov}); full-text search and XBRL company-facts endpoints & Every US public company's own filings; ties directly to Chapters 4 and 16 \\
...
\section{Credit, Lending, Insurance, and Consumer Finance Data}
\label{sec:appendix-credit-data}

\index{UCI Machine Learning Repository}This is the category with the deepest overlap with this book's own worked examples. The UCI ``Default of Credit Card Clients'' dataset is already Chapter 21's own running case study, downloaded programmatically by that chapter's notebook rather than committed to this repository. The other datasets in Table~\ref{tab:appendix-credit} are natural extensions for the capstone project ideas Chapter 21 closes with.
...
Source & Access & API / library & Notes \\
\midrule
UCI Default of Credit Card Clients & Free & Direct download via \texttt{urllib} & Chapter 21's own dataset; Taiwanese credit-card default data \\
...
Lending Club loan data & Free (via Kaggle mirror) & Kaggle API & Loan-level peer-to-peer lending data; a Chapter 21 capstone idea \\
HMDA (Home Mortgage Disclosure Act) data & Free & REST API via the Consumer Financial Protection Bureau's data browser (\url{consumerfinance.gov}) & Mortgage application-level data widely used for fair-lending analysis; directly usable for Chapter 18's fairness audits \\
French Motor Third-Party Liability (\texttt{freMTPL2freq}) & Free & OpenML (\texttt{fetch\_openml}) or the \texttt{CASdatasets} R package & The standard actuarial ratemaking benchmark: roughly 678{,}000 real motor policies with claim counts and rating features; Chapter 11's challenge exercise \\
\bottomrule
\end{tabular}
\end{table}
...
\index{IEEE-CIS Fraud Detection dataset}Genuinely labeled fraud data is scarce, since financial institutions rarely release real transaction-level fraud labels, which is exactly why the two datasets in Table~\ref{tab:appendix-fraud} recur so often in both industry benchmarks and this book's own Chapter 15 and Chapter 21 capstone list. Operational-risk loss data proper is even scarcer: the industry's event-level loss databases (the ORX consortium foremost among them) are member-only, which is why Chapter 10's real-data challenge falls back on the public enforcement and complaint text sources in the same table, regulatory documents rich in exactly the operational-loss and misconduct narratives that a bank's internal incident data contains.
...
Source & Access & API / library & Notes \\
\midrule
IEEE-CIS Fraud Detection (Kaggle) & Free (Kaggle competition dataset) & Kaggle API & Real, anonymized e-commerce transaction fraud data; a Chapter 15/21 capstone idea \\
PaySim & Free & Kaggle API & Synthetic mobile-money transaction data modeled on a real provider's logs; well suited to practicing the structuring and layering detection introduced in Chapter 15 \\
...
CFPB Consumer Complaint Database & Free & REST API and CSV export via the Consumer Financial Protection Bureau (\url{consumerfinance.gov}) & Millions of consumer complaints about financial products, many with free-text narratives; the proxy Chapter 10's challenge solution uses for classifying operational-risk narratives by category \\
SEC litigation releases and administrative proceedings & Free & Bulk index files and full-text search via \url{sec.gov} & Enforcement-action texts rich in operational-loss and misconduct narratives; the other real-text source Chapter 10's challenge exercise suggests \\
\bottomrule
\end{tabular}
\end{table}
...
\index{Loughran-McDonald dictionary}Chapter 16's treatment of sentiment scoring rests on the Loughran-McDonald Master Dictionary, a word list purpose-built for financial text (ordinary sentiment dictionaries misclassify routine financial vocabulary, such as ``liability'' or ``tax,'' as negative), freely downloadable from its maintainer's University of Notre Dame page. Table~\ref{tab:appendix-text} lists this and the other text sources most relevant to that chapter.
...
Source & Access & API / library & Notes \\
\midrule
SEC EDGAR full-text search & Free & REST API (\url{data.sec.gov}) & Search across filing text; directly usable for Chapter 16's 10-K risk-factor classification example \\
Loughran-McDonald Master Dictionary & Free & Direct CSV/text download & The sentiment word list underlying Chapter 16's scoring examples \\
...
Orbital Insight, RS Metrics & Enterprise, paid & Vendor portal/API & Satellite-derived alternative data referenced in Chapter 17's case vignette; not accessible without a commercial contract \\
...
Source & Access & API / library & Notes \\
\midrule
LOBSTER & Free academic tier + paid & Web portal, bulk download (\url{lobsterdata.com}) & Reconstructed limit order book data for NASDAQ-listed stocks; directly usable for Chapters 3, 13, and 14 \\
...
WRDS: NYSE TAQ & Institutional subscription & Web query tool, plus SAS/Python/R client & Tick-by-tick trade and quote data; the traditional academic source for Chapter 14-style microstructure research \\
\bottomrule
\end{tabular}
\end{table}
...
Source & Access & API / library & Notes \\
\midrule
Ken French Data Library & Free & Direct CSV/ZIP download; also wrapped by \texttt{pandas-datareader} & Fama-French factor returns and industry portfolios; directly usable for Chapter 12 \\
iShares, SPDR ETF holdings & Free & Daily CSV download from the issuer's own website & Actual ETF basket composition; usable for Chapter 14's ETF-basket arbitrage example \\
...
Table~\ref{tab:appendix-chapter-map} condenses the sections above into a single reference: for each chapter of this book, which of the sources above is the most natural place to look for real data to extend that chapter's fictional examples. Chapter 21's own closing list of capstone project ideas already names several of these sources directly; this table extends that same mapping to every other chapter.
...
10. Operational Risk Modeling & CFPB Consumer Complaint Database, SEC litigation releases and administrative proceedings \\
11. Insurance and Actuarial Modeling & French Motor Third-Party Liability (\texttt{freMTPL2freq}, via OpenML or \texttt{CASdatasets}) \\
12. Portfolio Theory and Asset Allocation & Ken French Data Library, Yahoo Finance \\
13. Algorithmic Trading and Market Microstructure & LOBSTER, Yahoo Finance intraday data (where available) \\
14. High-Frequency Trading & LOBSTER, Databento, WRDS NYSE TAQ \\
15. Fraud Detection, Compliance, and Financial Crime & IEEE-CIS Fraud Detection, PaySim, OFAC sanctions lists \\
16. Natural Language Processing in Finance & SEC EDGAR full-text search, Loughran-McDonald dictionary, Financial PhraseBank, GDELT, Reddit API \\
17. Alternative Data and Deep Learning in Finance & Google Trends, satellite/alt-data vendors (enterprise), Similarweb \\
18. Explainability, Fairness, and Regulation & HMDA data, UCI Default of Credit Card Clients (as in Chapter 21's own capstone fairness audit) \\
19. Reinforcement Learning in Finance & Simulated market environments by design; LOBSTER or Yahoo Finance intraday data for a real extension of the chapter's execution and market-making agents \\
20. Agent-Based Models in Finance & Simulated agent populations by design; LOBSTER order-book data for a real zero-intelligence-trader comparison, or BIS locational banking statistics for a real interbank exposure network \\
21. Case Studies and Capstone Projects & The full list above; see Chapter 21's own closing section for specific project pairings \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Point-in-time and look-ahead bias.} Most free APIs return a security's or company's data as it is known \emph{today}, not as it was actually known on the date being studied. Fundamentals get restated after the fact, index membership changes, and a company's own name or ticker can change; naively joining ``current'' fundamentals to historical prices silently leaks future information into a backtest, exactly the look-ahead-bias problem Chapter 17 raises directly.
