...
\section{A Taxonomy of Trading Strategies and Signals}
\label{sec:taxonomy}
...
\section{Order Types and the Mechanics of Execution}
...
\section{Market Impact and the Cost of Trading Fast}
...
\section{The Kyle Model: A Formal Model of Price Impact}
...
\section{TWAP and VWAP: Basic Execution Algorithms}
\label{sec:twap-vwap}
...
\end{equation*}
with $T=N\tau$ the total execution horizon; $\kappa$ is the single parameter that governs how front-loaded the optimal trajectory is, growing with risk aversion $\lambda$ and volatility $\sigma$ and shrinking with the impact cost $\eta$. The trajectory is stated here without proof; this book's companion technical note on the Almgren-Chriss model derives it from the discrete first-order condition, which turns out to be a linear second-order difference equation, and verifies the two limiting cases that make the result trustworthy: uniform, TWAP-style trading as risk aversion vanishes, and immediate liquidation as it grows without bound.
...
\end{equation*}
two dollars below the mid price, reflecting the market maker's desire to reduce its long position. Both halves of the spread formula above are quoted rather than derived; this book's companion technical note on the Avellaneda-Stoikov model obtains each exactly and separately, the reservation price from exponential (CARA) utility applied to a normally distributed terminal mark-to-market value, and the per-side markup from the Poisson arrival intensity, which is what makes the total spread decompose into an inventory-risk term that vanishes as the horizon closes and an order-arrival term that does not. The optimal spread works out to $\delta \approx \$1.69$, giving
\begin{align*}
...
\subsection{Tokenized Assets, Stablecoins, and New Settlement Rails}
\label{sec:tokenization}

The AMM machinery above assumed the assets being traded already live on a blockchain. The more consequential institutional development is the reverse direction: moving \emph{traditional} assets onto those rails. Asset tokenization\index{tokenization (of assets)} means issuing a conventional security, a money-market fund share, a Treasury bill, a bond, as a digital token whose ownership record is the blockchain itself rather than an entry at a custodian or transfer agent, and by the mid-2020s this had moved from concept to production: major asset managers now run tokenized money-market funds whose shares settle on public blockchains, and several government-bond issues have been placed natively in tokenized form.

The economics driving this are the settlement economics of Chapter 3, not speculative ones. A conventional securities trade settles through the multi-day, multi-intermediary chain of clearinghouse, custodian, and transfer agent described there, with each intermediary charging for its role and the delay itself creating the counterparty risk that central clearing exists to manage. A tokenized security can settle in minutes, around the clock, with delivery and payment executed atomically in a single transaction, no failed leg possible, which collapses both the cost and the counterparty-risk window. The catch is that atomic settlement needs both legs on-chain, which is where stablecoins\index{stablecoin} enter. These are tokens engineered to hold a fixed value, almost always \$1, by holding reserves of cash and short-term Treasuries against every token issued, functioning as the cash leg of on-chain markets. A stablecoin is economically a money-market fund share that has renounced yield in exchange for functioning as money. The quality of its reserves is exactly the credit question Chapter 9 would ask of any issuer, a question made vivid whenever a stablecoin has ``broken the buck'' and traded below its peg in a stress episode. Central banks have studied issuing the sovereign version, a central bank digital currency (CBDC), though as of this writing large-economy deployments remain pilots rather than production systems.

For the quantitative reader, the relevance is threefold. Tokenized markets generate exactly the granular, timestamped, public transaction data this chapter's methods consume, but around the clock and with every trade visible. The market-making and execution problems on these venues are the same Kyle-model and inventory-management problems developed above, transplanted onto AMM mechanics. And the migration itself is an institutional-plumbing shift of the kind Chapter 3 argued quantitative practitioners ignore at their peril. A settlement cycle collapsing from days to minutes changes collateral needs, intraday liquidity management, and the very meaning of counterparty exposure.
...
\subsection{A Worked Example: The Cost of Searching}
\label{sec:backtest-overfit}
\index{backtest overfitting}\index{selection bias}

Both pitfalls above are properties of the data. The third, and in practice the most damaging, is a property of the researcher. Section~\ref{sec:ma-crossover} reported a Sharpe ratio of $-0.22$ for a 3-day/6-day crossover, and almost nobody stops there: the natural next step is a 5-day/20-day pair, then a 10-day/50-day pair, then a stop-loss rule, reporting whichever combination came out best. That is not a backtest of one strategy. It is a search across many, and the figure it reports is the maximum of many noisy estimates rather than any single strategy's edge.

For a strategy with no genuine edge, observed over $T$ periods, the estimated Sharpe ratio is approximately normal with mean zero and standard error $1/\sqrt{T}$ per period, so on annualizing
\begin{equation*}
\text{SE}(\widehat{\text{SR}}_{\text{annual}}) \approx \frac{1}{\sqrt{\text{years of data}}},
\end{equation*}
and five years of daily data gives $1/\sqrt{5} = 0.447$. A single worthless strategy clears the conventional one-sided $5\%$ test whenever its measured Sharpe ratio exceeds $1.645 \times 0.447 = 0.74$, which by construction happens one time in twenty. Search $N$ configurations and the reported figure is no longer one draw but the largest of $N$, whose expectation grows without bound in $N$; Table~\ref{tab:backtest_search} gives that expected best Sharpe ratio alongside the hurdle the maximum must clear for a $5\%$ test of the search as a whole.

\begin{table}[htbp]
\centering
\caption{Best-of-$N$ in-sample Sharpe ratios when every strategy searched is worthless, five years of daily data}
\label{tab:backtest_search}
\begin{tabular}{rrrr}
\toprule
Configurations $N$ & $\mathbb{E}[\max]$ in $z$ & Expected best Sharpe & Honest $5\%$ hurdle \\
\midrule
1    & 0.00 & 0.00 & 0.74 \\
10   & 1.54 & 0.69 & 1.15 \\
100  & 2.51 & 1.12 & 1.47 \\
...
Take $N = 100$: five short windows, five long windows, and four exit rules, a search most researchers would call modest. The expected best in-sample annualized Sharpe ratio is $1.12$ even though every one of the hundred strategies is worthless by construction, and a Monte Carlo confirms the arithmetic, simulating one hundred zero-edge strategies over five years of daily returns ten thousand times for a mean best in-sample Sharpe ratio of $1.118$ against the analytic $1.121$. That figure clears the naive $0.74$ hurdle comfortably and would be presented in most settings as a strong result, whereas the honest hurdle for the best of one hundred is $1.47$, almost exactly twice the naive one. Out of sample the question is settled: the winning strategy earns a mean out-of-sample Sharpe ratio of $+0.004$, loses money outright $49.6\%$ of the time, and shows a rank correlation between in-sample and out-of-sample Sharpe ratios, across the hundred, of $+0.001$. The winner of a search is not better than what it beat. It is luckier, and luck does not persist into the next sample.

Three responses follow, and none of them is to stop searching. Report $N$, since a backtest presented without the number of configurations tried cannot be evaluated at all. Deflate the reported figure against the best-of-$N$ distribution rather than against zero, which is precisely what the deflated Sharpe ratio\index{deflated Sharpe ratio} of Bailey and L\'opez de Prado does, adjusting for the number of trials together with the non-normality of real strategy returns. And recognize that neighboring grid points are highly correlated, since a 49-day and a 50-day moving average produce nearly identical positions, so the effective number of independent trials sits well below the raw count: rerunning the simulation with a pairwise correlation of $0.5$ lowers the mean best Sharpe ratio to $0.80$, equivalent to about eighteen independent trials rather than a hundred. The correction is milder than the raw grid size implies, then, but nowhere close to absent. What none of it can manufacture is data: for a strategy whose \emph{true} annualized Sharpe ratio is $0.5$, a genuine and respectable edge, separating it from zero in a single pre-specified test takes about eleven years of daily returns, and clearing a hundred-configuration search takes about forty-three. Very little strategy research runs on anything approaching that history, which is the strongest argument available for pre-specifying a few economically motivated candidates, as Section~\ref{sec:taxonomy} framed them, rather than letting a grid search decide what the hypothesis should have been.

\section{Implementation Shortfall and Transaction Cost Analysis}
...
\end{equation*}
or \$70 in total on the 1,000-share order. This single number bundles together every friction discussed in this chapter: the market impact of Section~\ref{sec:market-impact}, the timing risk of Section~\ref{sec:optimal-execution}, and any pure price drift that occurred between the decision and its execution for reasons unrelated to the trade itself. It is the standard metric used in transaction cost analysis (TCA) to evaluate whether a trading desk's execution quality is actually adding or subtracting value relative to the investment decisions it is implementing. A strategy that looks profitable using decision prices alone can turn out to be unprofitable once realistic implementation shortfall is subtracted from its theoretical returns. That is exactly the trap Chapter 3 warned against when discussing why market structure matters for quantitative finance. It is also why serious backtesting, extending the performance metrics of Section~\ref{sec:backtest-metrics}, should model transaction costs explicitly rather than assume execution at the exact prices a signal was computed from.
...
\begin{itemize}
    ...
    \item Bailey and L\'opez de Prado, ``The Deflated Sharpe Ratio,'' \emph{Journal of Portfolio Management}. The source of the deflation adjustment recommended in Section~\ref{sec:backtest-overfit}, and the standard reference on backtest overfitting.
