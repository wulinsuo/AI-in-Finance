...
\section{What Makes Insurance Different}
...
\section{How an Insurer Makes Money: Float and the Combined Ratio}
...
\centering
\begin{lrbox}{\bookmeasuredbox}
\begin{tikzpicture}[node distance=5mm and 12mm, every node/.style={font=\small}]
...
\node[draw, rounded corners, fill=orange!12, minimum width=3.0cm, minimum height=0.9cm, align=center, below=14mm of float] (income) {Operating income =\\ underwriting result + investment income};
\draw[->] (prem) -- (float);
\draw[->] (float) -- (claims);
\draw[->] (float) -- (income);
...
\section{The Classical Pricing Baseline: Frequency Times Severity}
...
\end{equation*}
expected claim frequency times expected claim severity, and the art of ratemaking is estimating how each component varies with observable characteristics of the risk.
...
\section{GLM Ratemaking: The Traditional Approach}
\label{sec:ins-glm}
...
\section{Credibility: The Actuary's Bayesian Shrinkage}
...
\section{From GLMs to Machine Learning in Pricing}
...
\subsection{A Worked Example: The Interaction a GLM Cannot Price}
...
\subsection{Where Machine Learning Improves on the Traditional Approach, and Where It Does Not}
...
\section{Telematics and Usage-Based Insurance}
...
\section{Claims: Triage, Automation, and Fraud}
...
\section{Life Insurance, Mortality, and Longevity}
\label{sec:ins-life}
...
\section{Health Insurance and Risk Adjustment}
...
\subsection{Parametric Insurance and Basis Risk}
...
\section{Solvency: Capital Regulation as a Value at Risk}
...
\section{Fairness and Proxy Discrimination}
